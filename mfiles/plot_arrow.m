...
% source:  http://www.itl.nist.gov/div898/winds/wind_pressure/m-files/plot_arrow.m
...
if (nargin==0)
    ...
    for x = 0:0.3:2*pi
        ...
        h = plot_arrow( 1,1,50*rand*cos(x),50*rand*sin(x),...
            'color',color,'facecolor',color,'edgecolor',color, 'headwidth', 0.9 );
        set( h,'linewidth',2 );
    end
    hold off;
    return
...
if ~isempty( varargin )
    for c = 1:floor(length(varargin)/2)
        try
            ...
            case 'headheight',alpha = max( min( varargin{c*2},1 ),0.00 );
            case 'headwidth', beta = max( min( varargin{c*2},1 ),0.00 );
            end
        catch
